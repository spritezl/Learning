      ...
      INSERT /*+PARALLEL*/
        ...
        VALUES
          ...
          SELECT /*+PARALLEL(F)*/
           ...
           GROUPING_ID(YMWD.SM_SMONTH_ID, YMWD.SW_SWEEK_ID) TIME_GROUPING_ID,
           YMWD.SM_SMONTH_ID SMONTH_ID,
           YMWD.SW_SWEEK_ID SWEEK_ID,
           F.STORE_ID,
           ...
           INNER JOIN V_DIM_TIME_7TH_YMWD YMWD
              ON F.DATE_ID = YMWD.DATE_ID
           ...
           GROUP BY F.SALES_STYLE_ID,
                    ...
                    GROUPING         SETS(YMWD.SW_SWEEK_ID, YMWD.SM_SMONTH_ID)
      ...
      INSERT /*PARALLEL*/
        ...
        VALUES
          ...
          SELECT /*+PARALLEL(F)*/
           ...
           -- BOM
           SUM(CASE
                 WHEN YMWD.START_DATE = YMWD.SM_START_DATE THEN
           ...
           -- BOW
           SUM(CASE
                 WHEN YMWD.START_DATE = YMWD.SW_START_DATE THEN
                  F.BEGIN_INVENTORY_UNIT
                 ELSE
                  0
               END) BOW_UNIT,
           ...
           --EOM
           SUM(CASE
                 WHEN YMWD.END_DATE = YMWD.SM_END_DATE THEN
           ...
           -- EOW
           SUM(CASE
                 WHEN YMWD.END_DATE = YMWD.SW_END_DATE THEN
                  F.END_INVENTORY_UNIT
                 ELSE
                  0
               END) EOW_UNIT,
...
/*        EXECUTE IMMEDIATE 'ALTER TABLE FACT_ERROR_LOG TRUNCATE PARTITION FOR('||TO_CHAR(ITEM.UPLOAD_ID)||')';
    ...
    ELSE
      ...
      SELECT SET(CAST(COLLECT(S.UPLOAD_ID) AS NNT_IDS)),
             SET(CAST(COLLECT(YMWD.SM_SMONTH_ID) AS NNT_IDS)),
             SET(CAST(COLLECT(YMWD.SW_SWEEK_ID) AS NNT_IDS)),
             SET(CAST(COLLECT(YMWD.DATE_ID) AS NNT_IDS))
