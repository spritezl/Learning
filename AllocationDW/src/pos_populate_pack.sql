  ...
  BEGIN
    ...
    ELSIF P_REC_DATE_ATTRS.TRANSACTION_DATE IS NOT NULL THEN
      ...
      ELSE
        ...
        P_SMONTH_ID:=L_CACHE_SMONTH(TO_CHAR(P_DATE_ID));
      END IF;
    ELSE
...
BEGIN
  ...
  SELECT B.SMONTH_ID ID, TO_CHAR(A.DATE_ID) SOURCE_BUSKEY
    ...
    INTO L_TEMP_CACHE
    FROM DIM_DATE_DAY A
    INNER JOIN DIM_DATE_SWEEK B
    ON A.SWEEK_ID=B.SWEEK_ID;

  FOR J IN L_TEMP_CACHE.FIRST .. L_TEMP_CACHE.LAST LOOP
    L_CACHE_SMONTH(L_TEMP_CACHE(J).SOURCE_BUSKEY) := L_TEMP_CACHE(J).ID;
  END LOOP;

  SELECT S.MONTH_ID ID, TO_CHAR(S.MONTH_ID) SOURCE_BUSKEY
    BULK COLLECT
    INTO L_TEMP_CACHE
    FROM DIM_DATE_MONTH S;
