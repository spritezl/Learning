...
import Programming_Python.getfile_ftp as getfile
import os
import sys
...
class FtpForm(Form):
    ...
    def onSubmit(self):
        ...
        showinfo(self.title, '%s of "%s"' % (self.mode, filename))
...
class FtpGetfileForm(FtpForm):
    ...
    def do_tranfer(self, filename, servername, remotedir, userinfo):
